...
- Use R around 4 years
- Developing shiny based package for ML and forecasting application
...
- Offering an open source packages for interactive data visualization based on D3.js
...
- Orginazing the PLOTCON conference 
...
- Using loop to add or create new objects in plotly, could save time when we have multiple/reputable process    
...
-	Useful when we want to plot objects with different scale or measurement units
...
- Creating line for each year in the data using loop (time saver!) 
...
- Its seems like that the vehicle sales pick was during 2015 
- Plotly provide a dynamic legend
...
- visualize those values vs. the actual values could provide us more insights about the performance of the model beside the MAPE or the RMSE values
